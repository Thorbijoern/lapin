...
impl<T: AsyncRead+AsyncWrite+Send+Sync+'static> Channel<T> {
    /// create a channel
    pub fn create(transport: Arc<Mutex<AMQPTransport<T>>>) -> impl Future<Output = Result<Self, io::Error>> + Send + 'static {
        let channel_transport = transport.clone();

        future::poll_fn(move |ctx| {
            let mut transport = lock_transport!(channel_transport, ctx);
            if let Some(id) = transport.conn.create_channel() {
                ...
                return Poll::Ready(Err(io::Error::new(io::ErrorKind::ConnectionAborted, "The maximum number of channels for this connection has been reached")));
            }
        }).and_then(|channel| {
            channel.into_future()
        }).and_then(|channel| {
            ...
            channel.run_on_locked_transport("create", "Could not create channel", move |transport| {
                ...
                future::poll_fn(move |ctx| {
                    let transport = lock_transport!(transport, ctx);

                    match transport.conn.get_state(channel_id) {
                        Some(ChannelState::Connected) => return Poll::Ready(Ok(())),
                        Some(ChannelState::Error)     => return Poll::Ready(Err(io::Error::new(io::ErrorKind::Other, format!("Failed to open channel")))),
                        Some(ChannelState::Closed)    => return Poll::Ready(Err(io::Error::new(io::ErrorKind::Other, format!("Failed to open channel")))),
                        _                             => {
                            ctx.waker().wake();
                            return Poll::Pending;
                        }
    ...
    pub fn access_request(&self, realm: &str, options: AccessRequestOptions) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
    ...
    pub fn exchange_declare(&self, name: &str, exchange_type: &str, options: ExchangeDeclareOptions, arguments: FieldTable) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    pub fn exchange_delete(&self, name: &str, options: ExchangeDeleteOptions) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
        let name = name.to_string();
    ...
    pub fn exchange_bind(&self, destination: &str, source: &str, routing_key: &str, options: ExchangeBindOptions, arguments: FieldTable) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    pub fn exchange_unbind(&self, destination: &str, source: &str, routing_key: &str, options: ExchangeUnbindOptions, arguments: FieldTable) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
        let destination = destination.to_string();
        let source = source.to_string();
        let routing_key = routing_key.to_string();
    ...
    pub fn queue_declare(&self, name: &str, options: QueueDeclareOptions, arguments: FieldTable) -> impl Future<Output = Result<Queue, io::Error>> + Send + 'static {
        let channel_id = self.id;
        let name = name.to_string();
        ...
        self.run_on_locked_transport("queue_declare", "Could not declare queue", move |transport| {
            ...
            future::poll_fn(move |ctx| {
              ...
              if let Some(queue) = transport.conn.get_generated_name(request_id.expect("expected request_id")) {
                ...
                return Poll::Ready(Ok(Queue::new(queue, consumer_count, message_count)))
              } else {
                ctx.waker().wake();
                return Poll::Pending
              }
    ...
    pub fn queue_bind(&self, name: &str, exchange: &str, routing_key: &str, options: QueueBindOptions, arguments: FieldTable) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
        let name = name.to_string();
    ...
    pub fn queue_unbind(&self, name: &str, exchange: &str, routing_key: &str, options: QueueUnbindOptions, arguments: FieldTable) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
        let name = name.to_string();
        let exchange = exchange.to_string();
        let routing_key = routing_key.to_string();
    ...
    pub fn basic_qos(&self, options: BasicQosOptions) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    pub fn basic_publish(&self, exchange: &str, routing_key: &str, payload: Vec<u8>, options: BasicPublishOptions, properties: BasicProperties) -> impl Future<Output = Result<Option<RequestId>, io::Error>> + Send + 'static {
        let channel_id = self.id;
        let exchange = exchange.to_string();
        let routing_key = routing_key.to_string();
    ...
    pub fn basic_consume(&self, queue: &Queue, consumer_tag: &str, options: BasicConsumeOptions, arguments: FieldTable) -> impl Future<Output = Result<Consumer<T>, io::Error>> + Send + 'static {
        let channel_id = self.id;
        let transport = self.transport.clone();
        ...
        self.run_on_locked_transport("basic_consume", "Could not start consumer", move |transport| {
            ...
            future::poll_fn(move |ctx| {
              ...
              if let Some(consumer_tag) = transport.conn.get_generated_name(request_id.expect("expected request_id")) {
                return Poll::Ready(Ok(consumer_tag))
              } else {
                ctx.waker().wake();
                return Poll::Pending
              }
    ...
    /// acks a message
    pub fn basic_ack(&self, delivery_tag: u64, multiple: bool) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    /// nacks a message
    pub fn basic_nack(&self, delivery_tag: u64, multiple: bool, requeue: bool) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    /// rejects a message
    pub fn basic_reject(&self, delivery_tag: u64, requeue: bool) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    /// gets a message
    pub fn basic_get(&self, queue: &str, options: BasicGetOptions) -> impl Future<Output = Result<BasicGetMessage, io::Error>> + Send + 'static {
        let channel_id = self.id;
        let _queue = queue.to_string();
        let queue = queue.to_string();
        let receive_transport = self.transport.clone();
        let receive_future = future::poll_fn(move |ctx| {
            let mut transport = lock_transport!(receive_transport, ctx);
            transport.poll()?;
    ...
    pub fn queue_purge(&self, queue_name: &str, options: QueuePurgeOptions) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    pub fn queue_delete(&self, queue_name: &str, options: QueueDeleteOptions) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
        let queue_name = queue_name.to_string();
    ...
    /// closes the channel
    pub fn close(&self, code: u16, message: &str) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    /// ack a channel close
    pub fn close_ok(&self) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    pub fn channel_flow(&self, options: ChannelFlowOptions) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    pub fn channel_flow_ok(&self, options: ChannelFlowOptions) -> impl Future<Output = Result<(), io::Error>> + Send + 'static {
        let channel_id = self.id;
    ...
    fn run_on_locked_transport_full<Action, Finished>(&self, method: &str, error: &str, action: Action, finished: Finished, payload: Option<(Vec<u8>, BasicProperties)>) -> impl Future<Output = Result<Option<RequestId>, io::Error>> + Send + 'static
        where Action:   'static + Send + FnOnce(&mut AMQPTransport<T>) -> Result<Option<RequestId>, lapin_async::error::Error>,
              Finished: 'static + Send + Fn(&mut Connection, RequestId, &mut task::Context) -> Poll<Result<Option<RequestId>, io::Error>> {
        ...
        future::poll_fn(move |ctx| {
            ...
            // Once the lock_transport yields a Poll::Ready transport, the rest of the function is
            // ran only once as we either return an error or an Poll::Ready, it's thus safe to .take().unwrap()
            ...
            future::poll_fn(move |ctx| {
                let mut transport = lock_transport!(_transport, ctx);

                if let Some(request_id) = request_id {
                    Self::wait_for_answer(&mut transport, request_id, &finished, &ctx)
                } else {
    ...
    fn run_on_lock_transport_basic_finished(conn: &mut Connection, request_id: RequestId, ctx: &mut task::Context) -> Poll<Result<Option<RequestId>, io::Error>> {
        match conn.is_finished(request_id) {
            Some(answer) if answer => Poll::Ready(Ok(Some(request_id))),
            _                      => {
                ctx.waker().wake();
                Poll::Pending
            }
    ...
    fn run_on_locked_transport<Action>(&self, method: &str, error: &str, action: Action) -> impl Future<Output = Result<Option<RequestId>, io::Error>> + Send + 'static
        where Action: 'static + Send + FnOnce(&mut AMQPTransport<T>) -> Result<Option<RequestId>, lapin_async::error::Error> {
    ...
    pub fn wait_for_answer<Finished>(tr: &mut AMQPTransport<T>, request_id: RequestId, finished: &Finished, ctx: &mut task::Context) -> Poll<Result<Option<RequestId>, io::Error>>
        where Finished: 'static + Send + Fn(&mut Connection, RequestId, &mut task::Context) -> Poll<Result<Option<RequestId>, io::Error>> {
            trace!("wait for answer; request_id={:?}", request_id);
            tr.poll(ctx)?;
            trace!("wait for answer transport poll; request_id={:?} status=Pending", request_id);
            if let Poll::Ready(r) = finished(&mut tr.conn, request_id)? {
                trace!("wait for answer; request_id={:?} status=Ready result={:?}", request_id, r);
                return Poll::Ready(Ok(r));
            }
            trace!("wait for answer; request_id={:?} status=Pending", request_id);
            ctx.waker().wake();
            Poll::Pending
    }
}
